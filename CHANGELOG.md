...
- 🔥 移除 `BatchMemoryCleaner` 节点（批量显存清理）
- 🔥 移除 `ARESVRAMTrigger` 节点（VRAM 触发）
- 🔥 移除 `MemoryCleaner.clear_all_caches` 深度清理方法

> 移除节点的功能已合并进 `ReservedMemorySetter`（智能显存预留）节点，不再需要单独的监控/清理/Trigger 节点。
...
| 维度 | v2.0.0 | v2.2.0 |
|------|--------|--------|
| 预留接口 | EXTRA_RESERVED_MEMORY（失效） | EXTRA_RESERVED_VRAM（生效） |
| 节点数量 | 3 | 1（功能集成） |
| 监控/清理 | 独立节点 | 合并进预留节点 |
